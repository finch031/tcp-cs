...
public class Connection {
     ...
     * Will return -1 if this connection has never been connected
     ...
     * {@link #getReturnTripTime() return trip time}.
    ...
    /**
     * Returns the last calculated TCP return trip time,
     * or -1 if {@link #updateReturnTripTime()} has never been called or the
     ...
     * Periodically sending a keep alive ensures that an abnormal close is detected in a reasonable amount of time
     * (see {@link #setTimeout(int)}). Also,
     * some network hardware will close a TCP connection that ceases to transmit for a period of time
     * (typically 1+minutes). Set to zero to disable. Defaults to 8000.
     **/
    ...
    /**
     * Workaround for broken NIO networking on Android 1.6. If true,
     * the underlying NIO buffer is always copied to the beginning of
     * the buffer before being given to the SocketChannel for sending.
     * The Harmony SocketChannel implementation in Android 1.6
     * ignores the buffer position, always copying from the beginning of the buffer.
     * This is fixed in Android 2.0+.
     * */
     ...
     * This is returned by {@link #toString()} and is useful for providing application
     * specific identifying information in the logging.
     * May be null for the default name of "Connection X", where X is the
     * connection ID.
     **/
